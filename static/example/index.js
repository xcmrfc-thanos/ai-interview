...
(function () {
  ...
  userInputText = "";  // 新增变量，存储用户手动输入的内容
...
//      userInputText = "";  // 清空手动输入
  ...
  function renderResult(resultData) {
    ...
    if (jsonData.action == "started") {
     ...
     resultTextarea.value = userInputText + resultText + resultTextTemp;
  ...
  resultTextarea.addEventListener('input', function (event) {
    if (btnStatus === "OPEN") {
      ...
      resultTextarea.value = resultText + resultTextTemp;
    }
  });

   // **修改手动输入逻辑，确保手动输入不会被清空**
  resultTextarea.addEventListener('input', function () {
    userInputText = resultTextarea.value;  // 更新用户手动输入的内容
  });
